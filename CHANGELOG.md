...
- `VotingRustBackend.restoreRecoveredDelegation(_:)`, `RecoveredDelegationRestoreRequest`,
  ...
  holds the package. Each bundle carries `van`, the commitment the recovered row held; the call
  refuses a bundle whose blinding and weight do not open it before clearing anything. The
  request takes a `VotingHotkey`; its secret is unwrapped only at the FFI boundary, and the
  recovered blinding factors are `Undescribable`.
