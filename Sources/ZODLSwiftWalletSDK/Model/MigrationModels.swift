...
public struct MigrationBroadcastInstruction: Equatable, Sendable {
    ...
    /// `import ZODLSwiftWalletSDK` cannot name this initializer — possession of an instruction
    /// proves the caller cranked. TEST targets (the SDK's own, and downstream apps') opt in with
    /// `@_spi(Testing) import ZODLSwiftWalletSDK`: the SPI name is the ceremony that keeps the
...
public struct MigrationProveTarget: Equatable, Sendable {
    ...
    /// `@_spi(Testing) import ZODLSwiftWalletSDK` — the ceremony import that must never appear
