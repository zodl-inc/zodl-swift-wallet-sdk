...
public actor SlipstreamSynchronizer: Synchronizer {
    ...
    private func unifiedWalletSummary() async -> WalletSummary? {
        ...
        if snap?.isRecovering != 1 && snap?.tipFresh != 1 {
            // Otherwise the mask hides all spendable without a trace, and a transient
            // "funds pending / cannot send max" report cannot be diagnosed after the fact.
            initializer.logger.info("[#1591] stale-tip mask applied, tipFresh=\(snap?.tipFresh.description ?? "nil")")
