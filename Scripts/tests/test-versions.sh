...
test_repo_slug_from_ssh_url() {
    assert_eq "zodl-inc/zcash-swift-wallet-sdk" \
        "$(repo_slug_from_url 'git@github.com:zodl-inc/zcash-swift-wallet-sdk.git')" \
        "repo_slug_from_url scp-style ssh"
}

test_repo_slug_from_https_url() {
    assert_eq "zodl-inc/zcash-swift-wallet-sdk" \
        "$(repo_slug_from_url 'https://github.com/zodl-inc/zcash-swift-wallet-sdk.git')" \
        "repo_slug_from_url https with .git"
}

test_repo_slug_from_https_url_without_suffix() {
    assert_eq "zodl-inc/zcash-swift-wallet-sdk" \
        "$(repo_slug_from_url 'https://github.com/zodl-inc/zcash-swift-wallet-sdk')" \
        "repo_slug_from_url https without .git"
}

test_repo_slug_from_ssh_protocol_url() {
    assert_eq "zodl-inc/zcash-swift-wallet-sdk" \
        "$(repo_slug_from_url 'ssh://git@github.com/zodl-inc/zcash-swift-wallet-sdk.git')" \
